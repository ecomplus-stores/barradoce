import {
  ...
  export default {
    ...
    computed: {
      ...
      pictureCard () {
        const { pictures } = this.body
        const modifiedPicture = pictures.map(item => {
          const keys = Object.keys(item)
          const newPicture = keys.map(key => {
            if (key === 'normal') {
              item[key].size = '350x350'
            } else if (key === 'big') {
              item[key].size = '700x700'
            } else if (key === 'zoom') {
              item[key].size = '800x800'
            }
          })
          return item
        })
